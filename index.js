...
function getArtistByIndex(arr, desiredNumber) {
  return `the artist at index ${desiredNumber} is ${arr[desiredNumber].name}`
}
  /*Your Code Here*/
...
function get20s(arr){
  let higherYears = []
  for (let item of arr){
    if (parseInt(item.years) >= 1900){ 
      higherYears.push(item.name)
    }
  }
  return higherYears
}
